...
// codecRoundTripNode runs a node body through encode/decode using a
// fresh in-memory engine. Used by codec round-trip tests that don't
// care about anything beyond the codec itself.
func codecRoundTripNode(t *testing.T, n *Node) (*Node, error) {
	t.Helper()
	eng := newTestEngine(t)
	namespace := namespaceForNodeID(n.ID)
	var data []byte
	err := eng.withUpdate(func(txn *badger.Txn) error {
		var encErr error
		data, _, encErr = eng.encodeNodeInTxn(txn, namespace, n)
		if encErr != nil {
			return encErr
		}
	...
	return eng.decodeNode(namespace, data)
}

// buildV2NodeBody encodes a Node through the engine's V2 codec and
// returns the bytes. Used by tests that store synthetic node bodies
// directly under prefixNode (bypassing CreateNode) to set up specific
// decode-path scenarios.
func buildV2NodeBody(t *testing.T, engine *BadgerEngine, n *Node) ([]byte, error) {
	t.Helper()
	namespace := namespaceForNodeID(n.ID)
	var data []byte
	err := engine.withUpdate(func(txn *badger.Txn) error {
		var encErr error
		data, _, encErr = engine.encodeNodeInTxn(txn, namespace, n)
		if encErr != nil {
			return encErr
		}
...
// codecRoundTripEdge runs an edge body through encode/decode using a
// fresh in-memory engine. Allocates endpoint numIDs the same way the
// production write path does.
func codecRoundTripEdge(t *testing.T, e *Edge) (*Edge, error) {
	t.Helper()
	eng := newTestEngine(t)
	namespace := namespaceForEdgeID(e.ID)
	var data []byte
	err := eng.withUpdate(func(txn *badger.Txn) error {
		var encErr error
		data, encErr = eng.encodeEdgeInTxn(txn, namespace, e)
		if encErr != nil {
			return encErr
		}
	...
	return eng.decodeEdgeBodyWithID(namespace, data, e.ID)
}
