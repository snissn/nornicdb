...
- **MERGE on a uniquely-constrained value is now deterministically idempotent under concurrent writers.** Two writers MERGE-ing on the same `uid` would intermittently surface Badger's generic `"Transaction Conflict"` instead of the consumer-pinned commit-time UNIQUE shape (regression: `TestMERGE_IsIdempotentUnderConcurrentRetry`, ~0.05% under stress). Root cause: MERGE's MATCH-then-UPDATE flow could match a peer's just-committed node (because `GetNodesByLabel` runs through a fresh read txn, not the writer's `readTS`-bound view). The subsequent `OpUpdateNode` flowed into `writeNodeMVCCHeadInTxn`, which read the existing `mvccNodeHeadKey` via the user txn to carry `FloorVersion` forward; that read entered Badger's SSI read set, and when the peer committed the same key Badger rejected this writer with `Transaction Conflict`, skipping the constraint-violation classifier the cypher executor relies on. Fixed by routing the FloorVersion-carry-forward and archive-pre-overwrite reads through fresh read transactions via new `loadNodeMVCCHead` / `loadEdgeMVCCHead` helpers, and by routing `getNodeFromBadgerSnapshotLocked`'s primary-key fallback through `db.View`. The user txn no longer accumulates head or peer-primary keys in its SSI read set. Verified at 50,000 stress iterations.
...
- **Pre-pin reads lazy-bind the readTS to the namespace's actual sequence.** Without this, a transaction that reads before any write carries `readTS.CommitSequence == 0`, fails the `version.Compare(head.FloorVersion) < 0` visibility gate against any committed head, and the `Read → Modify → Write → Conflict-Retry` loop in `DB.Update` silently loses increments under contention. Caught by `TestDB_UpdateConcurrentIncrements`.
...
- **Commits in range**: 31 (non-merge)
- **Repository delta**: 205 files changed, +12,571 / -17,993 lines
- **Primary focus areas**: per-namespace MVCC isolation, MERGE-under-contention correctness (no more spurious Badger Transaction Conflict for commit-time UNIQUE losers), discoverable consumer documentation surface (skills + migration scripts + unified Bolt error shape), and `discover` similarity-vs-RRF correctness.
